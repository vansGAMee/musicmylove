...
export function dot(a: readonly number[], b: readonly number[]): number {
  let n=0; for(let i=0;i<a.length;i++) n+=a[i]*(b[i]??0); return n;
}
export function unit(v: number[]): number[] { const n=Math.sqrt(dot(v,v)); return n ? v.map(x=>x/n) : v; }
const order=(a:Hit,b:Hit)=>b.score-a.score||a.id-b.id;
function insert(list:Hit[], hit:Hit, limit:number) {
  let lo=0,hi=list.length; while(lo<hi){const mid=(lo+hi)>>>1;if(order(list[mid],hit)<=0)lo=mid+1;else hi=mid;}
  if(lo<limit){list.splice(lo,0,hit);if(list.length>limit)list.pop();}
}
function layer(index:Hnsw, vectors:readonly (readonly number[])[], q:readonly number[], entry:number, level:number, ef:number):Hit[] {
  ...
  while(queue.length){
    ...
    for(const id of index.levels[current.id]?.[level]??[]){
      ...
      if(best.length<ef||order(hit,best[best.length-1])<0){insert(best,hit,ef);insert(queue,hit,ef);}
    }
  }
  return best;
}
export function searchHnsw(index:Hnsw, vectors:readonly (readonly number[])[], q:readonly number[], k:number, ef=Math.max(64,k)):Hit[]{
...
export function buildHnsw(vectors:readonly (readonly number[])[], m=12, ef=64):Hnsw {
