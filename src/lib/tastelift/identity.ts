...
const VERSION_WORD = /\b(?:\d{4}\s+)?(?:remaster(?:ed)?|remix(?:ed)?|live|demo|acoustic|instrumental|radio\s+edit|single\s+version|album\s+version|mono|stereo|clean|explicit|cover)(?:\s+\d{4})?\b/iu;
const DASH_VERSION = new RegExp(`\\s*[-–—]\\s*(?:${VERSION_WORD.source})[^]*$`, "iu");
...
export function versionlessTitle(title: string): string {
  let result = normalizeIdentityText(title).replace(PARENTHETICAL, (whole, qualifier: string) => VERSION_WORD.test(qualifier) ? "" : whole);
