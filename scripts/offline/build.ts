...
// The legacy vectors are intentionally ignored. Graph is built from ListenBrainz histories + real web tracklists.
...
// Optional real audio vectors: generated by embed_audio.py or acousticbrainz_audio.py, never synthesized.
...
await writeFile('public/data/manifest.json',JSON.stringify({format:'human-graph-manifest-v1',sha256,file:'catalog.json',tracks:catalog.tracks.length,histories:allHistories.length,audio:catalog.audio?.ids.length??0,source:catalog.source}));
console.log(JSON.stringify({tracks:catalog.tracks.length,histories:allHistories.length,audio:catalog.audio?.ids.length??0,source:catalog.source}));
