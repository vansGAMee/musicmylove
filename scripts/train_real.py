...
def main():
    ...
    random.seed(41); torch.manual_seed(41); torch.use_deterministic_algorithms(True)
    model = TinyRanker(17)
    ...
    for epoch in range(1, 151):
        ...
        if epoch % 5 == 0:
            ...
            if learned_best > best_ndcg + 1e-6:
                best_ndcg, best_state, stale = learned_best, copy.deepcopy(model.state_dict()), 0
            else:
                stale += 1
            model.train()
            if stale >= 8:
    ...
    report = {
        ...
        "validation_examples": len(validation_evaluations), "history": history,
    ...
    print(json.dumps({key: value for key, value in report.items() if key not in ("history", "validation_metrics")} | {"selected_validation_ndcg": max(row["ndcg_at_20"] for row in validation.values())}))
